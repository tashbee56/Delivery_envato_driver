...
import 'package:deliverydriver/pages/NavigatorPages/editprofile.dart';
import 'package:deliverydriver/pages/NavigatorPages/fleetdocuments.dart';
import 'package:deliverydriver/pages/NavigatorPages/history.dart';
import 'package:deliverydriver/pages/NavigatorPages/makecomplaint.dart';
import 'package:deliverydriver/pages/login/get_started.dart';
import 'package:deliverydriver/pages/login/login.dart';
import 'package:deliverydriver/pages/login/otp_page.dart';
import 'package:deliverydriver/pages/login/ownerregister.dart';
...
import 'package:deliverydriver/pages/onTripPage/review_page.dart';
import 'package:deliverydriver/pages/vehicleInformations/referral_code.dart';
import 'package:deliverydriver/pages/vehicleInformations/service_area.dart';
import 'package:deliverydriver/pages/vehicleInformations/upload_docs.dart';
import 'package:deliverydriver/pages/vehicleInformations/vehicle_color.dart';
import 'package:deliverydriver/pages/vehicleInformations/vehicle_make.dart';
import 'package:deliverydriver/pages/vehicleInformations/vehicle_model.dart';
import 'package:deliverydriver/pages/vehicleInformations/vehicle_number.dart';
import 'package:deliverydriver/pages/vehicleInformations/vehicle_type.dart';
import 'package:http/http.dart' as http;
import 'package:firebase_auth/firebase_auth.dart';
import 'package:firebase_database/firebase_database.dart';
import 'package:firebase_messaging/firebase_messaging.dart';
import 'package:deliverydriver/pages/vehicleInformations/vehicle_year.dart';
...
//base url
String url =
    'base url'; //please add '/' in the end of url as 'yourwebsite.com/'
...
getVehicleModel() async {
  ...
  try {
    final response = await http.get(
      Uri.parse('${url}api/v1/common/car/models/$vehicleMakeId'),
    );
      ...
      if (userDetails['active'] == true &&
        ...
        try {
          firebase.child('drivers/${userDetails['id']}').update({
            ...
            'vehicle_type': userDetails['vehicle_type_id'].toString(),
          ...
          if (userDetails['active'] == true &&
              (userDetails['vehicle_type_id'] == null &&
                  userDetails['vehicle_types'].isEmpty)) {
            await driverStatus();
          }
          valueNotifierHome.incrementNotifier();
        } catch (e) {
...
userInactive() {
  ...
  firebase.child('drivers/${userDetails['id']}').update({
    'is_active': 0,
    'vehicle_type': (userDetails['vehicle_type_id'] != null)
        ? userDetails['vehicle_type_id']
        : userDetails['vehicle_types'].isNotEmpty
            ? jsonEncode(userDetails['vehicle_types'])
            : 'no vehicle',
  });
}
...
waitingBeforeStart() async {
  var bWaitingTime =
      await FirebaseDatabase.instance.ref('requests/${driverReq['id']}').get();
  var waitingTimes =
      await FirebaseDatabase.instance.ref('requests/${driverReq['id']}').get();
  if (bWaitingTime.child('waiting_time_before_start').value != null) {
    waitingBeforeTime = bWaitingTime.child('waiting_time_before_start').value;
  } else {
    waitingBeforeTime = 0;
  }
...
waitingAfterStart() async {
  ...
  if (aWaitingTime.child('waiting_time_after_start').value != null) {
    waitingAfterTime = aWaitingTime.child('waiting_time_after_start').value;
  } else {
    waitingAfterTime = 0;
  }
  await Future.delayed(const Duration(seconds: 10), () {});
...
geolocs.LocationSettings locationSettings = (platform == TargetPlatform.android)
    ...
    : geolocs.AppleSettings(
        accuracy: geolocs.LocationAccuracy.high,
        activityType: geolocs.ActivityType.automotiveNavigation,
        distanceFilter: 50,
